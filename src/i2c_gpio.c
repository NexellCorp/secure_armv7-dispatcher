...
//	return NX_GPIO_GetInputValue(g_I2C_GPIO_GRP, g_I2C_GPIO_SDA);
	return (CBOOL)((ReadIO32(&pReg_GPIO[g_I2C_GPIO_GRP]->GPIOxPAD)
				>> g_I2C_GPIO_SDA) & 1);
}
...
void I2C_Init(U8 gpioGRP, U8 gpioSCL,
	U8 gpioSDA, U32 gpioSCLAlt, U32 gpioSDAAlt)
{
	started = CFALSE;
...
#if 0
	ChangeIO32(&pReg_GPIO[g_I2C_GPIO_GRP]->GPIOxALTFN[g_I2C_GPIO_SCL >> 4],
			3 << ((g_I2C_GPIO_SCL & 0xF) << 1),
			NX_GPIO_PADFUNC_0 << ((g_I2C_GPIO_SCL & 0xF) << 1));
	ChangeIO32(&pReg_GPIO[g_I2C_GPIO_GRP]->GPIOxALTFN[g_I2C_GPIO_SDA >> 4],
...
//	printf("I2C_Deinit\r\n");
	...
	ChangeIO32(&pReg_GPIO[g_I2C_GPIO_GRP]->GPIOxALTFN[g_I2C_GPIO_SCL >> 4],
			3 << ((g_I2C_GPIO_SCL & 0xF) << 1),
			NX_GPIO_PADFUNC_0 << ((g_I2C_GPIO_SCL & 0xF) << 1));	// to gpio
	ChangeIO32(&pReg_GPIO[g_I2C_GPIO_GRP]->GPIOxALTFN[g_I2C_GPIO_SDA >> 4],
			3 << ((g_I2C_GPIO_SDA & 0xF) << 1),
			NX_GPIO_PADFUNC_0 << ((g_I2C_GPIO_SDA & 0xF) << 1));	// to gpio
#else
	ClearIO32(&pReg_GPIO[g_I2C_GPIO_GRP]->GPIOxALTFN[g_I2C_GPIO_SCL >> 4],
			NX_GPIO_PADFUNC_3 << ((g_I2C_GPIO_SCL & 0xF) << 1));
	SetIO32  (&pReg_GPIO[g_I2C_GPIO_GRP]->GPIOxALTFN[g_I2C_GPIO_SCL >> 4],
...
static CBOOL I2C_Start(void)
{
	...
	if (started) {
		...
		while (SCL_READ() == CFALSE) {	// clock stretching.... need timeout code
			if (timeout-- == 0) {
				ERROR("I2C_Start CLK Timeout Arbitration Fail\r\n");
				return CFALSE;
			}
			I2CDELAY(I2C_DELAY_TIME);
		}
	...
	if (SDA_READ() == CFALSE) {
		ERROR("I2C_Start SDA Arbitration Fail\r\n");
...
static CBOOL I2C_Stop(void)
{
	...
	while (SCL_READ() == CFALSE) { // need timeout code
		if (timeout-- == 0) {
			ERROR("I2C_Stop CLK Timeout Arbitration Fail\r\n");
			return CFALSE;
		}
		I2CDELAY(I2C_DELAY_TIME);
	}
	...
	while (SDA_READ() == CFALSE) {
		if (timeout-- == 0) {
			ERROR("I2C_Stop SDA Arbitration Fail\r\n");
			return CFALSE;	// arbitration_lost
		}
		I2CDELAY(I2C_DELAY_TIME);
	}
...
static CBOOL I2C_WriteBit(CBOOL Bit)
{
	...
	while (SCL_READ() == CFALSE) { // clock stretching.... need timeout code
		if (timeout-- == 0) {
			ERROR("I2C_WriteBit CLK Timeout Arbitration Fail\r\n");
			return CFALSE;
		}
		I2CDELAY(I2C_DELAY_TIME);
	}
	...
	if (Bit && (SDA_READ() == CFALSE)) {
		ERROR("I2C_WriteBit SDA Arbitration Fail\r\n");
		return CFALSE;	// arbitration_lost
	}
...
static CBOOL I2C_ReadBit(CBOOL *Bit)
{
	...
	while (SCL_READ() == CFALSE) {	// clock stretching.... need timeout code
		if (timeout-- == 0) {
			ERROR("I2C_ReadBit CLK Timeout Arbitration Fail\r\n");
			return CFALSE;
		}
		I2CDELAY(I2C_DELAY_TIME);
	}
...
static CBOOL I2C_WriteByte(
		CBOOL SendStart, CBOOL SendStop,
		U8 Data, CBOOL *nAck)
{
	U32 bit;
	CBOOL Arbitration;
	...
	if (Arbitration == CFALSE)
		return CFALSE;
	if (*nAck == CTRUE) {
		INFO("I2C_WriteByte nack returned\r\n");
		return CFALSE;
	}
...
CBOOL I2C_Read(U8 DeviceAddress, U8 RegisterAddress, U8 *pData, U32 Length)
{
	...
	INFO("I2C_Read %d\r\n", Length);
#endif
	result = I2C_WriteByte(CTRUE, CFALSE,
			DeviceAddress << 1 | I2CWRITE, &nAck);
	if (result == CFALSE) {
		ERROR("I2C Device Address Write Abitration Error\r\n");
		return CFALSE;
	}
	if (nAck == CTRUE) {
		ERROR("I2C Device Address Write Acknowledge Error\r\n");
		return CFALSE;
	}

	result = I2C_WriteByte(CFALSE, CFALSE, RegisterAddress, &nAck);
	if (result == CFALSE) {
		ERROR("I2C Register Address Write Abitration Error\r\n");
		return CFALSE;
	}
	if (nAck == CTRUE) {
		ERROR("I2C Register Address Write Acknowledge Error\r\n");
		return CFALSE;
	}
	result = I2C_WriteByte(CTRUE, CFALSE,
			DeviceAddress << 1 | I2CREAD, &nAck);
	if (result == CFALSE) {
		ERROR("I2C Device Address Write Abitration Error\r\n");
		return CFALSE;
	}
	...
	for (byte = 0; byte < Length; ) {
		byte++;
		result = I2C_ReadByte((byte == Length) ? CTRUE : CFALSE,
				(byte == Length) ? CTRUE : CFALSE, pData++);
		if (result == CFALSE) {
			ERROR("I2C Data Read Abitration Error\r\n");
			return CFALSE;
		}
	}
...
CBOOL I2C_Write(U8 DeviceAddress, U8 RegisterAddress, U8 *pData, U32 Length)
{
	...
	for (byte = 0; byte < Length;) {
		byte++;
		result = I2C_WriteByte(CFALSE,
				(byte == Length) ? CTRUE : CFALSE,
				*pData++, &nAck);
		if (result == CFALSE) {
			ERROR("I2C Data Write Abitration Error\r\n");
			return CFALSE;
		}
		if (nAck == CTRUE) {
			ERROR("I2C Data Write Acknowledge Error\r\n");
			return CFALSE;
		}
	}
